...
# 测试2
"""
limit1 = OceanHuedClam("nwr", 0).key_value(
    "name", "v-reg", "郁"
)  # 限制语句1：nwr["name"~"郁"]
set1 = Jellyfish("a", limit1)  # 将限制1作为集合a的条件
limit2 = (
    OceanHuedClam("node", 0)
    .import_jellyfish(set1)
    .in_jellyfish("a")
    .key_value("railway", "exist")
)
# 引用集合set1，并设定为在set1中检索限制语句2：nwr["railway"]
set2 = Jellyfish("b", limit2)
limit3 = (
    OceanHuedClam("nwr")
    .import_jellyfish(set2)
    .in_jellyfish("b")
    .key_value("public_transport", "exist")
    .key_value("train", "=", "yes")
    .timeout("100")
)
...
limit1 = (
    OceanHuedClam("nwr")
    .key_value("name:en", "v-Aa_no_care", "Shimo-Kitazawa")
    .timeout("25")
)
...
limit1 = (
    OceanHuedClam("node")
    .in_bbox("106.614", "106.613", "30.176", "30.177")
    .recurse("", "up")
)
